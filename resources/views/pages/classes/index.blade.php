...
@section('content')
    ...
    <div x-data="classes" class="xl:px-64 lg:px-40">
        <div class="panel mb-3">
            <h5 class="text-lg font-semibold dark:text-white-light mb-5">Create Class</h5>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <select class="form-select" x-model="stream_id" aria-label>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data("classes", () => ({
                ...
                init() {
                    axios.get('/api/grades').then(({data}) => {
                        if(data.status) this.grades = data.grades
                    })
                }
            }));
        })
